(function($) {
    ...
    $.fn.fb2 = function(){
      ...
      $.each($(fb).listAttributes(), function(index, value){
 	if (value.indexOf(":") != -1){
 	  ap = value.split(":")[1];
 	}
      });
	...
	description.titleinfo.coverpage = "";
	description.titleinfo.coverpage = $(titleinfo).find("coverpage").children("image").attr(ap+":href");
        description.titleinfo.coverpage = description.titleinfo.coverpage.substr(1);
	...
	if ( $(this).attr("name") != "notes" ) {
	  body += "<div class='book-body'><img class='cover-illustration' id=" + description.titleinfo.coverpage +" src='data:image/gif;base64," + getBinary(description.titleinfo.coverpage) + "' />";
    ...
    function sectionHtml(src,_level, isNotes){
	  ...
	  switch ((this).tagName) {
	    case 'image':
	      var id = $(this).attr(ap+":href").substr(1);
	      html += "<img class='section-illustration' id="+ id +" src='data:image/gif;base64," + getBinary(id) + "' />";
	      break;
	    case 'title':
	      if (isNotes && _level>0){
		$(this).children("p").each(function(){
		  html += "<a name="+$(src).attr("id")+"><h"+level+" class='section-title'>" + $(this).text() + "</h"+level + "></a>";
		});
		
	      }else{
		$(this).children("p").each(function(){
		  html += "<h"+level+" class='section-title'>" + $(this).text() + "</h"+level + ">";
		});
	      }
	      break;
	    case 'epigraph': {
	      html += "<p class='section-epigraph'>";
		...
		switch ((this).tagName) {
		  case 'p':
		    ...
		    break;
		  case 'text-author':
		    html += "<span class='section-epigraph-author'>" + $(this).text() + "</span><br/>";
		    break;
		  case 'empty-line':
		    html += "<br/>";
		    break;
		...
		$(this).children("a").each(function(){
		  $(this).text(" [<a href='"+$(this).attr(ap+":href")+"'>" + $(this).text() + "</a>]");
		});
    ...
    function getBinary(binaryId){
     //$("#main").append("<img src='data:image/gif;base64," + $(this).text() + "' />");
      var img = "";
      //description.img = []
      $(source).find("binary").each(function () {
	if ($(this).attr('id')==binaryId) {
	  //description.img.push($(this).text());
	  img = $(this).text();
	  //break;
	}
	//description.img.push($(this).text());
